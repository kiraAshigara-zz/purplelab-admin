var BRAINSPA_LAYOUT = {
    ...
    dataTable: function (fieldsInfo) {
        ...
        tableDOM.data('table-info', fieldsInfo);
    ...
    dataTableRow: function (row) {
        ...
        var tdTemplate = '<td class="tabledit-view-mode"><span class="tabledit-span" style="display: inline;">{1}</span><input class="tabledit-input form-control input-sm" type="text" name="{0}" value="{1}" style="display: none;" disabled=""></td>';
        ...
        $.each(row, function (field, value) {
            if (field === primaryKey) {
                tdDOM = $(noEditableTdTemplate.format(field, value));
            } else {
                tdDOM = $(tdTemplate.format(field, value));
            }
